...
children.push(
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 100 },
    children: [new TextRun({ text: '― 理論式ベース Sobol 感度解析（改訂第3版: 領域条件付き感度の頑健性検証）―', font: JP, size: 24, color: ACC })] }),
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: '作成日: 2026-07-21 ／ 改訂第3版', font: JP, size: 19, color: GREY })] }),
...
children.push(bullet('F-1 の残課題: 頑健性は拡散頻発域で確認したが、中間領域では penalty のみ抵抗が首位に来る（median/Spearman は密度）。領域境界近傍での連続性と、他時点（8年）での再現性は今後の検証対象。'));
...
children.push(p([new TextRun({text:'実行: ', font:JP, size:20, bold:true}), new TextRun({text:'python -m mf_demo.explorer.run_sensitivity 4096 2048（感度図）／ python -m mf_demo.explorer.run_explorer 600（①②③ 探索図）／ 領域条件付き・F-1・コンター条件比較は genfigs 系スクリプトで生成。', font:JP, size:19, color:GREY})]));
...
const doc = new Document({
  ...
  sections: [{
    ...
    footers: { default: new Footer({ children: [ new Paragraph({ alignment: AlignmentType.CENTER, children: [ new TextRun({ children: ['DC向けセル 保存劣化 感度解析報告書（改訂第3版） ／ ', PageNumber.CURRENT], font: JP, size: 16, color: GREY }) ] }) ] }) },
    children,
  }],
});
